...
# # --- LOAD SCORED DATA (no re-scoring here) ---
        ...
        try:
            ...
            for i, qa in enumerate(QA_pair):
                ...
                if user_ans and ref_ans:
                    ...
                    sim_score = float(util.cos_sim(emb_user, emb_ref).item())
                    sim_score = max(sim_score, 0.0)
                else:
                    sim_score = -1.0

                qa["cosine_similarity"] = sim_score
                similarities.append(sim_score)

                print(f"[DEBUG] Q{i+1}: Cosine similarity = {sim_score:.4f}")
